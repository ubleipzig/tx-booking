...

{
    ...
    function main()
    {
        ...
        if ($errors != '')
        {
            echo $this->pi_wrapInBaseClass($errors);
            return;
        }
        ...
        for ($o = 0; $o < count($data['uid']); $o++)
        {
            ...
            for ($i = 0; $i < 7; $i++)
            {
                ...
                if (!$this->conf['enableQuarterHourBooking'])
                {
                    ...
                    {
                        $bookingHours[].= date('H', $booking['startdate'][$h]);
                    }
                }
                ...
                if ($this->conf['enableQuarterHourBooking'])
                {
                    for ($h = 0; $h < count($booking['startdate']); $h++)
                    {
                        $bookingHours[].= date('H:i', $booking['startdate'][$h]);
                    }
                }
                ...
                {
                    $title = '';
                    if (!(array_search($h, $hours) === false)) $class = 'dutyHours';
                    else $class = 'offDuty';
                    if (!$this->conf['enableQuarterHourBooking'])
                    {
                        if (!(array_search($h < 10 ? '0' . $h : $h, $bookingHours) === false))
                        {
                            $class = 'bookedHours';
                            for ($bh = 0; $bh < count($bookingHours); $bh++)
                            {
                                if ($bookingHours[$bh] == $h) $title = ' alt="' . $this->getLL('bookedHour') . '" title="' . $this->getLL('bookedHour') . '"';
                            }
                        }
                    }
                    ...
                    if ($class == 'dutyHours' || $class == 'someQuarterAvailable') $onclick = 'tx_ubleipzigbooking_viewBookingForm(' . $this->_GP['year'] . ',' . $month . ',' . $day . ',' . $data['uid'][$o] . ',\'' . $ajaxData . '\');';
                ...
                for ($h = 12; $h < 24; $h++)
                {
                    ...
                    if (!$this->conf['enableQuarterHourBooking'])
                    {
                        if (!(array_search($h, $bookingHours) === false))
                        {
                            $class = 'bookedHours';
                            for ($bh = 0; $bh < count($bookingHours); $bh++)
                            {
        ...
        if (!$this->conf['enableQuarterHourBooking'])
        {
            for ($i = 0; $i < count($hours); $i++)
            {
                ...
                for ($j = 0; $j < count($data['feUserName']); $j++)
                {
                    if ($data['startdate'][$j] == $day + $hours[$i] * 3600)
                    {
                        ...
                        $marks['###MEMOV###'].= ' value="' . htmlspecialchars($data['memo'][$j]) . '"/>';
                            ...
                            $marks['###DELETEV###'].= '<img src="/typo3conf/ext/ubleipzigbooking/pi1/res/delete.png" alt="' . $this->getLL('delete') . '" title="' . $this->getLL('delete') . '" /></div>';
                            $marks['###MEMOV###'] = '<input type="text" id="tx_ubleipzigbooking_pi1_memo' . $i . '" class="memo" disabled="disabled"';
                ...
                if (!$occupied && $day + $hours[$i] * 3600 > time() && $GLOBALS['TSFE']->fe_user->user['uid'])
                {
                    ...
                    $marks['###BOOKV###'].= '<img src="/typo3conf/ext/ubleipzigbooking/pi1/res/book.png" alt="' . $this->getLL('book') . '" title="' . $this->getLL('book') . '" /></div>';
                }
                else $marks['###BOOKV###'] = '';
    ...
    function bookObject()
    {
        ...
        $where = ' feUserUid = ' . $this->conf['feUserUid'] . ' AND startdate > ' . $dayTime . ' AND startdate < ' . ($dayTime + 86400);
        $result = $GLOBALS['TYPO3_DB']->exec_SELECTquery($fields, 'tx_ubleipzigbooking', $where, '', '');
        while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result))
        {
            $counts = $row['counts'];
        }

        if ($this->conf['maxBookingsPerDay'] && $counts >= $this->conf['maxBookingsPerDay'])
        {
            list($day, $month, $year) = explode('.', $this->conf['date']);
            $this->_GP['day'] = $year . '-' . $month . '-' . $day . ':' . $this->conf['objectUid'];
            $this->viewBookingForm($this->getLL('maxBookingsPerDay'));
            return;
        }
        ...
        $fields = array(
            ...
            'memo' => strip_tags($this->_GP['day']) ,
            'cruser_id' => $this->conf['feUserUid'],
            'crdate' => time() ,
            'tstamp' => time() ,
        );
        $result = $GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_ubleipzigbooking', $fields);
