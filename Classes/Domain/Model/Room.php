...
class Room extends AbstractEntity
{
     ...
     */
    public const FOREIGNBOOKED = 8;

    /**
     * already booked by himself
     */
    public const OWNBOOKED = 16;
    ...
    public function getDayOccupation(Day $day)
    {
        foreach ($this->closingDays as $closingDay) {
            if ($closingDay->getDay()->getTimestamp() === $day->getTimestamp()) {
                return self::OFFDUTY;
            }
        }
    ...
    {
        if (!$this->isDayBookable(new Day($hour->getTimestamp()))) {
            return false;
        }
    ...
    public function getMinMaxOpeningHours(Day $day = null)
    {
        ...
        foreach ($this->openingHours as $dayOfWeek => $openingHours) {
            if ($day && ((int)$day->format('N') !== $dayOfWeek)) {
                continue;
            }
