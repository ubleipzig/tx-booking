...
class BookingController extends AbstractController
{
    ...
    public function addAction($room, $timestamp, $comment)
    {
        ...
        } elseif (!$this->settingsHelper->isAdmin() && $this->settingsHelper->exceededBookingLimit($timestamp)) {
            $this->addFlashMessageHelper('bookingInFuture', \TYPO3\CMS\Core\Messaging\FlashMessage::ERROR);
        } elseif (!$room->isHourBookable($hour)) {
            $this->addFlashMessageHelper('alreadyBookedByForeignUser', \TYPO3\CMS\Core\Messaging\FlashMessage::ERROR);
        } elseif (!$this->settingsHelper->isAdmin()
            && (count($this->bookingRepository->findByUserAndTime($GLOBALS['TSFE']->fe_user->user['uid'], $hour->getDateTime())) > 0)) {
            $this->addFlashMessageHelper('alreadyBookedInOtherRoom', \TYPO3\CMS\Core\Messaging\FlashMessage::ERROR);
        } elseif (!$this->settingsHelper->isAdmin() && ($this->settingsHelper->getMaxBookings()
                && count($this->bookingRepository->findByUserAndRoomsAndBetween($GLOBALS['TSFE']->fe_user->user['uid'], $rooms, $day->getStart(), $day->getEnd())) >= $this->settingsHelper->getMaxBookings())) {
    ...
    public function removeAction($room, $timestamp)
    {
        ...
        } elseif ($booking = $this->bookingRepository->findByUserAndRoomAndTime($GLOBALS['TSFE']->fe_user->user['uid'], $room, $hour->getDateTime())->getFirst()) {
